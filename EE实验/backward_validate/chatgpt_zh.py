...
def run_gold(input_list, input_list2eve, cots, mode):
    for idx, example in tqdm(enumerate(input_list), total=len(input_list), desc="Processing..."):
        ...
        while 1:
            try:
                rsp_1 = call_chatgpt(example, e_type, cot, 0.3) 
                rsp_2 = call_chatgpt(example, e_type, cot, 0.6)
                rsp_3 = call_chatgpt(example, e_type, cot, 1)
...
run(input_list, input_list2eve, event_extend_map, cots, 'zh')
run_gold(input_list, input_list2eve, cots,'zh')