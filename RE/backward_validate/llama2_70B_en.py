...
def run(input_list, input_list2rel, cot_spos,cot_examples,rel_extend_map,mode):
    for idx, example in tqdm(enumerate(input_list), total=len(input_list), desc="Processing..."):
        ...
        for r in rel_extend:
            msg_list=[
                {"role": "system", "message": get_sys_prompt(cot_example, cot_spo)},
                {"role": "user", "message": get_backward_prompt(example, r)},
            ]
            rsp_1 = call_llama(msg_list, 0.1) 
            # rsp_2 = call_alpaca(msg_list, 0.5)
#             rsp_3 = call_alpaca(msg_list, 1)
...
def run_gold(input_list, input_list2rel, cot_spos,cot_examples,mode):
    for idx, example in tqdm(enumerate(input_list), total=len(input_list), desc="Processing..."):
        ...
        msg_list=[
            {"role": "system", "message": get_sys_prompt(cot_example, cot_spo)},
            {"role": "user", "message": get_backward_prompt(example, rel)},
        ]
        rsp_1 = call_llama(msg_list, 0.1) 
...
# run(input_list, input_list2rel, cot_spos,cot_examples,rel_extend_map, 'llama2_70B_en')
run_gold(input_list, input_list2rel, cot_spos,cot_examples, 'llama2_70B_en')