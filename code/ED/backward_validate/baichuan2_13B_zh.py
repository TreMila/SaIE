...
def run_gold(input_list, input_list2eve, cots, mode):
    for idx, example in tqdm(enumerate(input_list), total=len(input_list), desc="Processing..."):
        ...
        msg_list=[
                {"role": "system", "content": get_sys_prompt(cot)},
                {"role": "user", "content": get_backward_prompt(example, event)},
            ]
        rsp_1 = call_api(msg_list,0.3) 
...
run(input_list, input_list2eve, event_extend_map, cots, 'baichuan2_13B_zh')
run_gold(input_list, input_list2eve, cots, 'baichuan2_13B_zh')