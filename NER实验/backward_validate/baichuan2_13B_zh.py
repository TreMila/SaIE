    ...
    for idx, example in tqdm(enumerate(input_list), total=len(input_list), desc="Processing..."):
        ...
        for e in ent_extend:
            msg_list=[
                ...
                {"role": "user", "content": get_backward_prompt(example, e)},
            ]
...
def run_gold(input_list, input_list2ent, cots, mode):
    for idx, example in tqdm(enumerate(input_list), total=len(input_list), desc="Processing..."):
        ent = entity_type_dict[input_list2ent[example]]
        cot = cots[idx]

        msg_list=[
            {"role": "system", "content": get_sys_prompt(cot)},
            {"role": "user", "content": get_backward_prompt(example, ent)},
        ]
        rsp_1 = call_api(msg_list,0.3)
...
run(input_list, input_list2ent, entity_extend_map, cots, 'baichuan2_13B_zh')
run_gold(input_list, input_list2ent, cots, 'baichuan2_13B_zh')