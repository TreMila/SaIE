...
def run_gold(input_list, input_list2ent, cots, mode):
    for idx, example in tqdm(enumerate(input_list), total=len(input_list), desc="Processing..."):
        ...
        prompt = get_sys_prompt(cot) + get_backward_prompt(example, ent)
        history = []
        rsp_1 = call_api(prompt, history, 0.3) 
...
run(input_list, input_list2ent, entity_extend_map, cots, 'chatglm_6B_zh')
run_gold(input_list, input_list2ent, cots, 'chatglm_6B_zh')